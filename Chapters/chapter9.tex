...
{\paradigmline{[\com]}{(\sab)}{}}
    ...
    \begin{itemize}
        \item Sazh
        \begin{itemize}
                \item Remove Doctors Code if no Fortisol
        \end{itemize}
    \end{itemize}
\end{itemize}
\end{menu}
...
\begin{itemize}
	...
	\item Summon
	\item Repeat, refresh with [4], Repeat until above 198\% Chain on the Reaver and full Gestalt
	\item Gestalt
	\item Thunderfalls (Circle/B) until the Reaver is above 506\% Chain
	\item Zantetsuken (Triangle/Y)
    \end{itemize}
\end{itemize}
\itemdrop{68.4}{Incentive Chip} \itemdrop{56.4}{Credit Chip} \itemdrop{5.0}{Cobaltite} \itemdrop{1.25}{Aegisol}
\end{battle}
Activate the bridge directly ahead. While the bridge is extending, activate a Deceptisol.  Run down the ramp about two steps past the first orange triangles on the sides. Return to the Bridge 1 enemies and cancel the Deceptisol at least a quarter of the way across the platform, thus despawning the Bridge 3 enemies. Repeat this procedure (activate bridge, respawn Bridge 2 enemies, cancel shroud, proceed forward). Menu after activating the final bridge. 
